...
$userPhoto = $stmt->fetchColumn() ?: 'assets/img/default.png';
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <?php if ($userRole === 'Superadmin'): ?>
            <div class="superadmin-section" style="margin-top: 40px; padding-top: 30px; border-top: 2px solid var(--border-color);">
                ...
                <div class="admin-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 20px;">
                    <!-- ADD HOSPITAL FORM -->
                    <div class="admin-card" style="background: var(--bg-card); padding: 20px; border-radius: 12px; border: 1px solid var(--border-color);">
                        <h4>Add New Hospital</h4>
                        <form id="addHospitalForm" style="margin-top: 15px;">
                            <input type="hidden" name="action" value="add_hospital">
                            <div class="input-field" style="margin-bottom: 10px; border: 1px solid var(--border-color); padding: 5px 15px; border-radius: 8px;">
                                <input type="text" name="name" placeholder="Hospital Name" required style="width: 100%; border: none; padding: 10px; background: transparent; color: var(--text-main);">
                            </div>
                            <div class="input-field" style="margin-bottom: 10px; border: 1px solid var(--border-color); padding: 5px 15px; border-radius: 8px;">
                                <input type="email" name="email" placeholder="Email Address" required style="width: 100%; border: none; padding: 10px; background: transparent; color: var(--text-main);">
                            </div>
                            <div class="input-field" style="margin-bottom: 10px; border: 1px solid var(--border-color); padding: 5px 15px; border-radius: 8px;">
                                <input type="password" name="password" placeholder="Password" required style="width: 100%; border: none; padding: 10px; background: transparent; color: var(--text-main);">
                            </div>
                            <div class="input-field" style="margin-bottom: 20px; border: 1px solid var(--border-color); padding: 5px 15px; border-radius: 8px;">
                                <input type="text" name="location" placeholder="Location" required style="width: 100%; border: none; padding: 10px; background: transparent; color: var(--text-main);">
                            </div>
                            <button type="submit" class="btn btn-admin" style="width: 100%; padding: 12px; background: var(--primary-color); color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">Register Hospital</button>
                        </form>
                    </div>
                ...
                document.getElementById('addHospitalForm').addEventListener('submit', async (e) => {
                    ...
                    const response = await fetch('../actions/superadmin_actions.php', {
                        method: 'POST',
                        body: formData
                    });
                ...
                async function deleteHospital(id) {
                    if (!confirm('Are you sure you want to delete this hospital? This will also delete the hospital login account.')) return;
