<script>
    ...
    function showBill(bookingId, doctorName, datetime, fee, isPaid = false) {
        ...
        if (isPaid) {
            titleEl.innerHTML = '<i class="fas fa-file-invoice-dollar"></i> Payment Receipt';
            ...
            titleEl.innerHTML = '<i class="fas fa-file-invoice-dollar"></i> Booking Invoice';
...
<script>
                ...
                <div class="pd-card" style="${showWarning ? 'border: 1px solid #fee2e2; background: #fffcfc;' : ''}">
                    ...
                    <div style="display:flex; flex-direction:column; gap:8px; margin-top: 15px;">
                        ...
                        ${a.status === 'scheduled' && !isInactive && !isLocked ? `
                            <div style="display:flex; gap:10px;">
                                ...
                                ${a.payment_status === 'paid' ? `
                                    <button class="pd-btn" style="flex:1; background:var(--primary-color); color:white;" onclick="refundAppt(${a.id})"><i class="fas fa-undo"></i> Refund</button>
                                ` : `
                        ...
                        ` : (a.status === 'pending_payment') ? `
                            <div style="display:flex; gap:10px;">
                                <button class="pd-btn" style="flex:1; background:var(--success); color:white;" onclick="retryPayment('${a.booking_id}')"><i class="fas fa-credit-card"></i> Pay Now</button>
                                <button class="pd-btn pd-btn-danger" style="flex:1;" onclick="cancelAppt(${a.id})">Cancel</button>
                            </div>
                        ` : ''}
                    </div>
    ...
    async function refundAppt(id) {
        const confirmed = await showConfirm({
            ...
            message: 'Are you sure you want to cancel this appointment and request a refund? The refund process may take 5-10 business days.',
    ...
    async function retryPayment(bookingId) {
        try {
            ...
            if (result.status === 'stripe_redirect') {
                ...
                showToast(result.message || 'Error initiating payment.', 'error');
            }
        } catch (e) {
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        if (paymentStatus === 'success') {
            ...
            if (bookingId) {
                // We'd need to fetch appointment details to show the bill correctly, 
                // or just wait for loadAppointments to finish and user can click it.
                // For better UX, we'll just let the toast handle the feedback and appointments list show the change.
            }

            // Remove the param from URL
            window.history.replaceState({}, document.title, window.location.pathname);
        } else if (paymentStatus === 'cancelled') {
            showToast('Payment was cancelled. Your appointment was not booked.', 'error');
            switchTab(null, 'book');
            // Remove the param from URL
            window.history.replaceState({}, document.title, window.location.pathname);
        }
